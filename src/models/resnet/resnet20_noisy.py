...
class BasicBlock:
    ...
    def __call__(self, x: Tensor):
        out = self.bn1(self.conv1(self.act1(x))).relu()
...
class ResNet:
    ...
    def to(self, device: str):
        for x in get_parameters(self) if device else []:
            x.to_(device)


def resnet20_cifar10_noisy(num_classes=10, pretrained=False):
    if not pretrained:
