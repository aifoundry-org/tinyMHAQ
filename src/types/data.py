...
class ImageClassificationDataset:
    ...
    def _load_disk_tensor(self, db_list, val_split=0.2, seed=None, postfix=""):
        ...
        Y_shape = (total_samples,) if first_y.ndim == 1 else (total_samples,) + first_y.shape[1:]
        ...
        if val_split == 0:
            X = Tensor(X_np[indices], device=f"disk:{self.dataset_path}/input_{postfix}")
            Y = Tensor(Y_np[indices], device=f"disk:{self.dataset_path}/target_{postfix}")
            return X, Y
        ...
        X_train = Tensor(X_np[train_indices], device=f"disk:{self.dataset_path}/input_train")
        Y_train = Tensor(Y_np[train_indices], device=f"disk:{self.dataset_path}/target_train")
